<script>
  ...
  function getTimeBetween(start, end) {
    ...
    return `${hours?hours + ":":""}${hours?minutes.toString().padStart(2, '0'):minutes.toString().padStart(1, '0')}:${seconds.toString().padStart(2, '0')}`;
  }

  function periodToString(period) {
    return (!isNaN(Number.parseInt(period))?"period ":"") 
      ...
      <div class="schedule">
        {#each periods as period}
          <Paper style="background-color: {getPeriodColor(period.period)}">
            ...
            <h3>{period.start.toLocaleTimeString("en-US", {hour:'2-digit', minute:'2-digit'})} - {period.end.toLocaleTimeString('en-US', {hour:'numeric', minute:'2-digit'})}</h3>
          </Paper>
        {/each}
      </div>
    </Paper>
